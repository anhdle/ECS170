...
public class AStarExp_997995396_998513706 implements AIModule
{
    ...
    private double getHeuristic(final TerrainMap map, final Point pt1, final Point pt2) {
        ...
        double goalH = map.getTile(pt2);
        double pH = map.getTile(pt1);
        
        double xDiff = Math.abs(pt2.x - pt1.x);
        double yDiff = Math.abs(pt2.y - pt1.y);
        double diff;
        double diffH = pH-goalH;
        
        if( xDiff < yDiff ) { diff=yDiff; } else { diff=xDiff; }
        
        if( diffH < 0 ) { // above endpoint
            
            return (-diffH * Math.E) + diff + diffH;
            
        } else if( diffH > 0 ) { // below endpoint
