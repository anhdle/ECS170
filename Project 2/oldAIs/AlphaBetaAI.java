...
public class AlphaBetaAI extends AIModule
{
    ...
    private int maxMove;
    private final int[] branchOrdering = {3, 2, 4, 1, 5, 0, 6};
    private final EvaluationTable ET_Current = new EvaluationTable();
    ...
    // go in the middle first!
    boolean goMiddleFirst = false;
    ...
    public void getNextMove(final GameStateModule game) {
        if( goMiddleFirst && game.getCoins() == 0 ) {
            chosenMove = 3;
            return;
        }
        
        ET_Current.opponentsMove(game); // update with opponents last move
        
        // Do Not Delete! this is the main call, it also prints the nash
        System.out.println( "\n===== Nash: " + alphaBeta(game, ET_Current, 0, 1, Integer.MIN_VALUE, Integer.MAX_VALUE) + " =====\n" );
        
        chosenMove = maxMove;
        ET_Current.move(maxMove, 1); // update with our move
    ...
    private int alphaBeta(final GameStateModule game, EvaluationTable ET, int depth, int playerIndex, int alpha, int beta){
        ...
        if(playerIndex ==  1){ // Max player
            for(int i : branchOrdering) // order form center out to maximize pruning
    ...
    public class EvaluationTable {
        ...
        private final HashMap<String, Integer> overlap = new HashMap<String, Integer>();
        private int score;
        ...
        public void move(int col, int player) {
            ...
            // update horizontal
            int l = p*h[row][col-1]; // check left
            int r = p*h[row][col+1]; // check right
            ...
            int bl = p*dp[row-1][col-1]; // check bottom left
            int tr = p*dp[row+1][col+1]; // check top right
            ...
            int br = p*dn[row-1][col+1]; // check bottom right
            int tl = p*dn[row+1][col-1]; // check top left
            ...
            } else if( br > 0 ) { // just bottom left
            
                for(int i=0; i<=br; i++)
                    dn[row-i][col+i] = p*br+p;
                
            } else if( tl > 0) { // just right
...
//            }
        ...
        // finds and add opponents move to the EvaluationTable
        ...
        // score table by adding up all 
        private void scoreTable() {
            String key = genHashKey(v);
            if( overlap.containsKey(key) ) {
                score = overlap.get(key);
                return;
            } 
            ...
            for(int i=1; i<=6; i++) 
                for(int j=1; j<=7; j++) {
                    ...
                    score += (v[i][j] + h[i][j] + dp[i][j] + dn[i][j])*(weights[i-1][j-1]); //*(weights[i-1][j-1])
                }
        ...
        // return a string version of the board for key
        private String genHashKey( int[][] a ) {
